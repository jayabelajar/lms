<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <x-button href="{{ route('admin.courses.show', $course) }}" variant="secondary" icon="arrow-left" class="w-full sm:w-auto">
                Kembali
            </x-button>
        </div>
    </x-slot>
    ...
    <x-card>
        ...
        <form method="POST" action="{{ route('admin.courses.update', $course) }}" class="space-y-6">
            ...
            <x-input label="Judul" name="title" type="text" icon="text-size" placeholder="Mata Kuliah title" required value="{{ old('title', $course->title) }}" />
            <x-input label="Slug" name="slug" type="text" icon="link" placeholder="slug-mata-kuliah" required value="{{ old('slug', $course->slug) }}" />
            <x-textarea label="Deskripsi" name="description" rows="5" icon="notes" placeholder="Deskripsi (opsional)">{{ old('description', $course->description) }}</x-textarea>
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                ...
                <x-button href="{{ route('admin.courses.show', $course) }}" variant="secondary" class="w-full sm:w-auto">Cancel</x-button>
            </div>
        </form>
    </x-card>
</x-app-layout>
