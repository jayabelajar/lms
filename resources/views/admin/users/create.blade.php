<x-app-layout>
    ...
    <x-card>
        <x-slot name="header">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between w-full">
                <div class="flex flex-col gap-1">
                    ...
                    <p class="text-[13px] font-medium text-gray-500 dark:text-gray-400">Kelola dan lihat informasi detail tentang add new user.</p>
                </div>
        ...
        <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-input label="NIP (Admin/Instructors)" name="nip" type="text" icon="id" placeholder="NIP" value="{{ old('nip') }}" />
                <x-input label="NIM (Students)" name="nim" type="text" icon="id" placeholder="NIM" value="{{ old('nim') }}" />
            </div>
